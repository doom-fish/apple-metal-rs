use apple_metal::{gpu_family, resource_options, MetalDevice, TextureDescriptor, pixel_format};

fn main() {
    let d = MetalDevice::system_default().expect("no Metal");
    println!("unified memory: {}", d.has_unified_memory());
    println!("recommended max working set: {} MB",
    ...
    let buf = d.new_buffer(4096, resource_options::STORAGE_MODE_SHARED)
    ...
    println!("texture {}x{} fmt={}", tx.width(), tx.height(), tx.pixel_format());
}
